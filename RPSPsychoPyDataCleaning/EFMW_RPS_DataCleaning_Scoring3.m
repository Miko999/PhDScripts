...
% New copy of this code because I am now adding calculation for reaction
% time variability for the MCT.
...
% task scores and descriptives are saved to the RPS_Task_Scores.csv and
% RPS_Practice_Scores.csv
...
%% Add reaction time relative to stimulus onset to MCT
    ...
    % Seli et al., 2013 "Rhythmic Response Times (RRTs) were first
    % calculated as the difference of the time of the key press and the onset 
    % of the metronome; the Mean RRT therefore indicates whether, on average, 
    % participants' responses precede or succeed the metronome tone."
        ...
        elseif ~isnan(SwitchPData.SwitchPAcc(SwitchPIdx))
            ...
            if strcmp('stay',SwitchPData.SwitchPCond(SwitchPIdx))
                ...
                if strcmp(SwitchPData.SwitchPResp(SwitchPIdx),SwitchPData.SwitchPCRespMixed(SwitchPIdx))
                    % accuracy check value is correct
                    SwitchPData.SwitchPAccuracyCheck(SwitchPIdx) = "correct";
                    % add rt to the correct reaction time storage
            ...
            elseif strcmp('switch',SwitchPData.SwitchPCond(SwitchPIdx))
                ...
                if strcmp(SwitchPData.SwitchPResp(SwitchPIdx),SwitchPData.SwitchPCRespMixed(SwitchPIdx))
                    % accuracy check value is correct
                    SwitchPData.SwitchPAccuracyCheck(SwitchPIdx) = "correct";
                    % add rt to the switch correct reaction time storage
                    SwitchPSwitchCorrRT = [SwitchPSwitchCorrRT; SwitchPData.SwitchPRT(SwitchPIdx)];
                    % add the the switch correct counter
                    SwitchPSwitchCorrTrials = SwitchPSwitchCorrTrials + 1;
                % if no response
                elseif strcmp('',SwitchPData.SwitchPResp(SwitchPIdx))
                    % accuracy check value is miss
                    SwitchPData.SwitchPAccuracyCheck(SwitchPIdx) = "miss";
...
%% Scoring actual switch task
    ...
    for SwitchIdx = 1:size(SwitchData,1)
        ...
        elseif ~isnan(SwitchData.SwitchAcc(SwitchIdx))
            ...
            if strcmp('stay',SwitchData.SwitchCond(SwitchIdx))
                ...
                % add rt to storage
                SwitchStayRT = [SwitchStayRT; SwitchData.SwitchRT(SwitchIdx)];
                % if they responded correctly
            ...
            elseif strcmp('switch',SwitchData.SwitchCond(SwitchIdx))
                ...
                % Add rt to storage
                SwitchSwitchRT = [SwitchSwitchRT; SwitchData.SwitchRT(SwitchIdx)];
                % if they responded correctly
                if strcmp(SwitchData.SwitchResp(SwitchIdx),SwitchData.SwitchCRespMixed(SwitchIdx))
                    % accuracy check value is correct
                    SwitchData.SwitchAccuracyCheck(SwitchIdx) = "correct";
                    ...
                    % add to accuracy counter
                    SwitchSwitchCorrectTrials = SwitchSwitchCorrectTrials + 1;
                % if no response
                elseif strcmp('',SwitchData.SwitchResp(SwitchIdx))
                    % accuracy check value is miss
                    SwitchData.SwitchAccuracyCheck(SwitchIdx) = "miss";
    ...
    SwitchStaySDRT = std(SwitchStayRT,"omitnan");
...
%% Scoring Sym Span Practice
    ...
    for SymPIdx = 1:size(SymSpanPData,1)
        ...
        if ~strcmp('',SymSpanPData.SymSpanPSymCResp(SymPIdx)) && ~strcmp('',SymSpanPData.SymSpanPRecStim(SymPIdx))
            ...
            if strcmp("correct",SymSpanPData.SymSpanPRecAccuracyCheck(SymPIdx))
                ...
                % add to correct rt storage
                SymSpanPRecMixedCorrRT = [SymSpanPRecMixedCorrRT; SymSpanPData.SymSpanPRecRT(SymPIdx)];
            end
        end
    ...
    if sum(isnan(SymSpanPSymOnlyRT))==size(SymSpanPSymOnlyRT,1)
        ...
        SymSpanPRecMixedCorrMeanRT = mean(SymSpanPRecMixedCorrRT,"omitnan");
        SymSpanPRecMixedCorrSDRT = std(SymSpanPRecMixedCorrRT,"omitnan");
    end
...
%% Scoring Actual SymSpan
    ...
    for SymIdx = 1:size(SymSpanData,1)
        ...
        if strcmp(SymSpanData.SymSpanMixRecStim(SymIdx),SymSpanData.SymSpanMixRecResp(SymIdx))
            ...
            % add to rt storage
            SymSpanMixRecCorrRT = [SymSpanMixRecCorrRT; SymSpanData.SymSpanMixRecRT(SymIdx)];
        else
    ...
    if sum(isnan(str2double(SymSpanData.SymSpanMixSymRT))) == size(SymSpanData.SymSpanMixSymRT,1)
        ...
        SymSpanMixRecCorrMeanRT = sum(str2double(SymSpanMixRecCorrRT));
        SymSpanMixRecCorrSDRT = sum(str2double(SymSpanMixRecCorrRT));
    else
        ...
        SymSpanMixRecCorrMeanRT = mean(SymSpanMixRecCorrRT,"omitnan");
        SymSpanMixRecCorrSDRT = std(SymSpanMixRecCorrRT,"omitnan");
    end
...
%% Reaction time variability notes

% Most studies using a form of the metronome task calculated response 
% time variability as the natural log transformed response variace in 
% the five trials before a probe. Response time is difference between probe onset and tap.


% Seli et al., 2013 doesn't say what they did for when the moving
% window reached the end. Anderson and Farb 2020 said calculating
% reaction time variance in the first five trials of each cycle
% necessarily includes trials from a previous cycle.
% Meier, 2018, lists out how the first window is trial 5,6,7,8,9;
% next is 6.7.8.9.10

% I am assuming they meant 'variance' as in the average of squared
% deviations from the mean.

% Wilson et al., 2022 looked at response time standard deivation.
% But also average key press timing.
% Seli, Jonker et al., 2013, "average of the transformed variance
% scores across the moving window...overall MRT RRT variance."
% Browsowsky et al., 2020 - computed variances in reaction time
% within each five-trial window. then normalized these scores. then
% average these transformed scores for an overall measure of rt
% variance


% Seli et al., 2013:
% "the primary measure of interest obtained from the MRT is the 
% Proportion of Mind Wandering (i.e., the number of probes to which a 
% participant reported mind wandering, divided by the total number of probes 
% presented)

% To ensure the MRT was generally being performed correctly, and that the 
% participants' self-reported mind wandering was indeed a departure from performing 
% the task at hand, we also calculated several task-related response measures.

% Rhythmic Response Times (RRTs) were first calculated as the difference of the time 
% of the key press and the onset of the metronome; the Mean RRT therefore indicates 
% whether, on average, participants' responses precede or succeed the metronome tone. 

% As variability in response time is the main measure of attention yielded by the MRT, 
% aside from probe-caught mind wandering, three measures of variance of the RRTs were also calculated. 

% Our first measure of variability, Mean RRT Variability, was computed using a moving window of 
% the current and preceding four trials across all trials throughout the task except the first 
% five trials and five trials after each probe (see Seli et al., 2013b)

% Our second measure of variability was for the five trials preceding subjective reports 
% of on-task performance (On-task RRT Variability).

% Finally, our third measure of variability involved the five trials preceding reports 
% of mind wandering (Mind Wandering RRT Variability).

% Variance data are often highly positively skewed, so we adjusted each variance measure 
% using a natural logarithm transform."

% I am assuming they calculated variability like variance as the average of squared deviations from the mean.
% Looking through OSF not everyone clearly shows how they calculated
% everything.
% Brosowsky et al., 2020 used the var() function in R (with mutate, group
% by, and filter to get certain trials). They also seemed to just exclude
% trials and make the sliding window go over all remaining trials?

% Planning
% Create storage for reaction times from trials besides the first five and
% the five after each probe

% create storage for reaction times for different thought probe
% types: MW, On task, Aware, Unaware, Intentional, Unintentional,
% MW Aware, MW Unaware, MW Int, MW Unint, On task Aware, On task Unaware,
% On task intent, on task unint
% MW aware int, MW unaware int, MW aware unint, MW unaware unint
% On task aware int, ON task unaware int, On task aware unint, on task
% unaware unint.

    % For each row starting at the 6th trial

        % if there is a probe in the current row, that means this is the
        % last trial before the probe appeared and the five trials after
        % should not be a part of variance calculations.

            % THIS DOESN'T WORK SINCE THE PROBED TRIAL WOULDN'T BE A PART
            % OF THE PROBING RT VARIANCE

            % determine the probe response

            % if the probe response is MW
                % store the MCTPRTDiff and MCTRTDiff in the MW storage
                % array
                    % if the probe response is also aware
                        % store in aware array
                        % and store in MW aware array
                            % if the probe is also intentional
                                % store in intentional array
                                % store in MW aware intentional array
                            % if the probe is also unintentional
                                % store in unintentional array
                                % store in MW aware unintentional array
                    % if the probe response is also unaware
                        % store in unaware array
                        % and store in MW unaware array
                            % if the probe is also intentional
                                % store in intentional array
                                % store in MW unaware intentional array
                            % if the probe is also unintentional
                                % store in unintentional array
                                % store in MW unaware unintentional array

            % else if the probe response is on task
                % store the MCTPRTDiff and MCTRTDiff in the on task storage
                % array
                    % if the probe response is also aware
                        % store in aware array
                        % and store in on task aware array
                            % if the probe is also intentional
                                % store in intentional array
                                % store in on task aware intentional array
                            % if the probe is also unintentional
                                % store in unintentional array
                                % store in on task aware unintentional array
                    % if the probe response is also unaware
                        % store in unaware array
                        % and store in on task unaware array
                            % if the probe is also intentional
                                % store in intentional array
                                % store in on task unaware intentional array
                            % if the probe is also unintentional
                                % store in unintentional array
                                % store in on task unaware unintentional array


            % end if loop for probe types
        
        % If there is a probe in any of the previous five trials

        % the five trials after


        % this would calculate the variance for groups of five but wouldn't
        % exclude the times after probes properly.
        % If there is no probe in the current row or any of the previous five trials
            % calculate variance for MCTPRTDiff and MCTRTDiff in the
            % current row combined with previous four
            % add this variance to the all variance storage
        % (If there is a probe in the previous five trials, move to next row)
        % If there is a probe in the current row
            % calculate variance for MCTPRTDiff and MCTRTDiff in the
            % current row combined with the previous four
    ...
    % Need to take all MCTPRTDiff and MCTRTDiff except the first five
    % trials and except the five trials after every probe. So have to
    % identify the rows with probes and then remove the five rows below
    % that row.

    % Could create a new array composed of MCTPRTDiff and MCTRTDiff and
    % probe responses
    % then remove the first five rows
    % then loop
    % store row numbers to exclude
        % for each row
            % if there is a probe
                % save the row number for the next five rows.
    % remove the row numbers that should be excluded.

    % then can calculate variance with a sliding window for all trials.
    % then subset the data by probe type and calculating variance with a
    % sliding window for the subset arrays.
...
%% Score Practice MCT
    ...
    for MCTPIdx = 1:(size(MCTPData,1))
        ...
        % To calculate response time variability based on probe responses,
        % we take the five trials before each probe
    ...
    % Store selection of MCT data

    MCTPRTData = table(MCTPData.MCTPRTDiff, MCTPData.MCTPProbeType, MCTPData.MCTPProbeOnOffResp, MCTPData.MCTPProbeAwareResp, ...
        MCTPData.MCTPProbeIntentResp, 'VariableNames', {'MCTPRTDiff','MCTProbeType','MCTPProbeOnOffResp','MCTPProbeAwareResp','MCTPProbeIntentResp'});
    % Remove to the first five trials
    MCTPRTData = (MCTPRTData(6:end,:));

    % Identify rows with probes
    MCTPRTProbeRows = [find(~isnan(MCTPRTData.MCTProbeType))];

    MCTPRTExclusionRows = [];
    
    % for each row with probes
    for PRowIdx = 1:length(MCTPRTProbeRows)
        % store the row number for the next five rows.
        MCTPRTExclusionRows = [MCTPRTExclusionRows; MCTPRTProbeRows(PRowIdx)+1; MCTPRTProbeRows(PRowIdx)+2; MCTPRTProbeRows(PRowIdx)+3;
        MCTPRTProbeRows(PRowIdx)+4; MCTPRTProbeRows(PRowIdx)+5];
    end

    % create storage for variance values
    MCTPRTVarianceVals = [];

    % for each row of trials
    % subtract 4 because going beyond the fifth last number goes out of
    % table dimensions
    for PTrialRowIdx = 1:(size(MCTPRTData,1)-4)
        % get the row numbers for the rows to calculate variance over
        RowNumbers = [PTrialRowIdx:PTrialRowIdx+4];
        % if these rows do not include any exclusion rows
        if 0 == sum(find(ismember(MCTPRTExclusionRows,RowNumbers)))
            % calculate variance for these rows
            VarianceValue = var(MCTPRTData.MCTPRTDiff(RowNumbers),"omitnan");
            % add this to variance storage
            MCTPRTVarianceVals = [MCTPRTVarianceVals;VarianceValue];
        end

        clear RowNumbers RTDiffVals VarianceValue

    end

    % create a new table with variance and probe responses
    MCTPRTProbeVarianceVals = array2table(zeros(0,4));
    MCTPRTProbeVarianceVals.Properties.VariableNames = {'ReactionTimeVariance','MCTPProbeOnOffResp','MCTPProbeAwareResp','MCTPProbeIntentResp'};

    % for each row of trials after the first four (to stay within table
    % dimensions
    for PTrialProbeRowIdx = 1:size(MCTPRTData,1)
        % if there is a probe
        if ~isnan(MCTPRTData.MCTProbeType(PTrialProbeRowIdx))
            % if the row number for the probe is less than five
            if PTrialProbeRowIdx < 5
            % get the row numbers for the current row back to probe 1
                RowNumbers = [1:PTrialProbeRowIdx];
            % otherwise
            else
            % get row numbers for current row and previous four
                RowNumbers = [PTrialProbeRowIdx-4:PTrialProbeRowIdx];
            end
            ...
            % calculate variance 
            VarianceValue = var(MCTPRTData.MCTPRTDiff(RowNumbers),"omitnan");
            % create a new table for values to add to storage table
            VarianceRow = [VarianceValue, MCTPRTData.MCTPProbeOnOffResp(PTrialProbeRowIdx),MCTPRTData.MCTPProbeAwareResp(PTrialProbeRowIdx),MCTPRTData.MCTPProbeIntentResp(PTrialProbeRowIdx)];
            % add to probe response storage table
            MCTPRTProbeVarianceVals = [MCTPRTProbeVarianceVals;VarianceRow];
            end
        end
        clear RowNumbers RTDiffVals VarianceValue VarianceRow
    end
    % then can calculate variance with a sliding window for all trials.
    % then subset the data by probe type and calculating variance with a
    % sliding window for the subset arrays.

    % If probe, add row number to vector,
    % in loop for moving window
    % if the five trials in the window include a row number in the vector,
...
writetable(PTaskScores, [maindir 'RPS_Practice_Scores.csv'])
